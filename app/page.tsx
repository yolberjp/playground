    ...
    <div className="flex flex-col pb-20 gap-10 px-10 sm:px-20 font-[family-name:var(--font-geist-sans)]">
      <header className="space-y-4">
        ...
        <p className="text-sm sm:text-lg text-gray-400">
          A space to put new things into practice and experiment without fear of
          breaking anything.
        </p>
      </header>
      ...
      <section className="space-y-6">
        <header>
          <h3 className="text-md italic text-gray-500">
            The last cooked components:
          </h3>
        </header>
        <ul className="px-10 md:px-20 space-y-3">
          {links.map((link) => (
            <li key={link.href} className="w-fit">
              <Link
                href={link.href}
                className="text-md hover:text-yellow-800 flex flex-row items-center gap-2"
              >
                <link.icon />
                {link.label}
              </Link>
